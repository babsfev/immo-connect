...
import { EmptyState } from "@/components/ui/EmptyState"; // Assurez-vous d'avoir ce composant
...
interface DataTableProps<T> {
  ...
  emptyMessage?: string; // <--- La prop manquante
}
...
export function DataTable<T extends { id: string | number }>({ 
  ...
  isLoading = false,
  onRowClick,
  emptyMessage = "Aucune donnée trouvée." // Valeur par défaut
}: DataTableProps<T>) {
    ...
    return (
      <div className={cn("rounded-2xl border border-slate-200 bg-white overflow-hidden", className)}>
         <EmptyState 
            ...
            description={emptyMessage} // Utilisation de la prop
         />
      </div>
    );
  }
