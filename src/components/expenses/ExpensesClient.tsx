...
export default function ExpensesClient({
  ...
  // State pour le fichier uploadé (pas encore envoyé au serveur)
  ...
  const handleSubmit = (formData: FormData) => {
    // Simulation: Si on avait un upload cloud, on enverrait l'URL ici.
    // Pour l'instant on ignore l'image côté serveur si l'action attend une string URL
  ...
  const recoverableRate =
    stats.totalAmount > 0
      ? Math.round((stats.recoverableAmount / stats.totalAmount) * 100)
      : 0;
        ...
        <Card className="border-l-4 border-l-blue-500">
          <CardContent className="p-6">
            ...
            <p className="text-xs text-blue-600 mt-2 font-medium">
              {formatCurrency(stats.recoverableAmount)} à refacturer aux
              locataires.
            </p>
          </CardContent>
        </Card>
          ...
          {expenses.length === 0 ? (
            ...
            expenses.map((exp: any) => {
              ...
              return (
                <div
                  key={exp.id}
                  className="p-4 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-slate-50 transition-colors group gap-4"
                >
                      ...
                      <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1.5">
                        ...
                        {exp.receiptUrl && (
                          <span className="text-blue-600 flex items-center">
                            <FileText size={10} className="mr-1" /> Reçu
                          </span>
                        )}
                      </p>
                    </div>
      ...
      >
        <form action={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input name="title" placeholder="Ex: Facture Senelec" required />
          </div>
            ...
            <div>
              ...
              <Select
                ...
                options={Object.entries(EXPENSE_CATEGORY_LABELS).map(
                  ([k, v]) => ({
                    label: v as string, // Force le type string
                    value: k,
                  })
                )}
                placeholder="Choisir..."
          ...
          <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-100">
            ...
            >
              Cette dépense est{" "}
              <span className="font-bold text-blue-600">récupérable</span>{" "}
              (Locataire)
            </label>
          </div>
